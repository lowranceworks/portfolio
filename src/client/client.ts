...
scene.add(joshCube)
joshCube.position.z = -4.3;
joshCube.position.x = 1.54;
...
function moveCamera() {
    ...
    joshCube.rotation.x += 0.01;
    joshCube.rotation.y -= 0.01;
    joshCube.rotation.z += 0.01;
...
function createPlanet(scene: THREE.Scene, planet: THREE.Mesh, group: THREE.Group, x: number, scale: number): void {
...
function createRing(color:THREE.Texture, innerRadius:number, outerRadius:number): THREE.Mesh {  
  const ring = new THREE.RingGeometry(innerRadius, outerRadius, 100, 8, 0, 6.283185307179586)
  const ringTexture = new THREE.TextureLoader().load('./img/ring-texture.jpg');
  const meshMaterial = new THREE.MeshStandardMaterial({ map: color, normalMap: ringTexture })
  const planetRing = new THREE.Mesh(ring, meshMaterial).rotateY(9.5).rotateX(-14.4).rotateZ(8);

  return planetRing;
}

function createPlanetaryRing(scene: THREE.Scene, ring: THREE.Mesh, group: THREE.Group, x: number ) {
